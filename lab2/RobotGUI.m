...
%
% It is adapted from a simple GUI by Phil Goddard found here:
...
% Version: 2.1
%
% Tested on MATLAB version: R2011b
...
try
    ...
    ha = axes('Parent',hf,...
        'HandleVisibility','callback',...
        'Unit','normalized',...
        'OuterPosition',[0.25 0.275 0.8 0.7],...
        'Xlim',[-0.075 0.3],...
        'YLim',[-0.075 0.225],...
        'Tag','plotAxes');
    xlabel(ha,'X');
    ...
    % Create a robot environment panel
        ...
        'Style','text',...
        'Units','normalized',...
        'Position',[0.125 0.3 0.6 0.25],...
        'Backgroundcolor',get(hf,'Color'),...
        'String','Wall Stiffness (N/mm):',...
        'HandleVisibility','callback',...
...
function robot = initializeRobot(ad)
...
robot.base = rectangle('Parent',ha,'Position',[-base -base 2*base 2*base],'Curvature',[0.1 0.1],'FaceColor','k');
robot.belt = line('Parent',ha,'XData',beltpoints(2,:),'YData',beltpoints(1,:),'Color',[0.7 0.5 0.25]);
robot.pulley1 = rectangle('Parent',ha,'Position',[-pulley/2 -pulley/2 pulley pulley],'Curvature',[1 1],'FaceColor',[0.5 0.5 0.5]);
robot.pulley2 = rectangle('Parent',ha,'Position',[-pulley/2 a1-pulley/2 pulley pulley],'Curvature',[1 1],'FaceColor',[0.5 0.5 0.5]);
robot.link2 = patch('Parent',ha,'XData',linkpoints(1,:),'YData',a1+linkpoints(2,:),'Facecolor',[0.8 0 0]);
robot.link1 = patch('Parent',ha,'XData',linkpoints(2,:),'YData',linkpoints(1,:),'FaceColor',[0.8 0 0]);
robot.joint1 = rectangle('Parent',ha,'Position',[-hole/2 -hole/2 hole hole],'Curvature',[1 1],'FaceColor',[0.8 0.8 0.8]);
robot.joint2 = rectangle('Parent',ha,'Position',[-hole/2 a1-hole/2 hole hole],'Curvature',[1 1],'FaceColor',[0.8 0.8 0.8]);
robot.tool = rectangle('Parent',ha,'Position',[a1-hole/2 a1-hole/2 hole hole],'Curvature',[1 1],'FaceColor','b','EdgeColor','w');
robot.path = line('Parent',ha,'XData',0.25,'YData',0,'Color','b');
text('Parent',ha,'Position',[0.2125 0.21],'String','Clock:');
robot.time = text('Parent',ha,'Position',[0.26 0.21],'String','0.00');
text('Parent',ha,'Position',[-0.05 -0.05],'String','Q','Color','w');
text('Parent',ha,'Position',[-0.039 -0.0525],'String','UANSER','Color','w','FontSize',6);
patch('Parent',ha,'XData',Uxpoints+0.04,'YData',Uypoints-0.055,'FaceColor',[0.8 0 0],'EdgeColor',[1 1 1]);
...
function localWallDisplayChecked(hObject,eventdata)
    ...
    if (checked)
        ...
        if (~isempty(find_system(ad.modelName,'Name','WallStiffness')))
            ...
            set(ad.handles.wallStiffnessEdit,'String','No wall found in model');
        end
    else
        set(ad.robot.wall,'Visible','off');
        set(ad.handles.wallStiffnessEdit,'String','');
